...
=======
   ...
   *
   * Finally, you may have heard the saying:
>>>>>>> fix errors in code. clarify intended compile errors. rewrote confused unit vector example
  ...
  trait Bird extends Ordered[Bird] {
    def name: String
    def compare(that: Bird): Int = name.compareTo(that.name)
  }
